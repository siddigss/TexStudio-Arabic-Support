...
{
    HKL  currentLanguage = getCurrentLanguage();
    QString RLMStr = QChar(0x200F);
    QString dirChar = LRMStr;
    if (((int)currentLanguage & 0x000000FF) == LANG_ARABIC){
        dirChar = RLMStr;
    }
    std::vector<QString> charsToCheck = {"\\", "^", "-", "_", "+", "=", "!", "/", "#", "%", "*", " "};
	const QString &text = event->text();
    bool S_test = true;//editor->cursor().isRTL();
    //if (editorViewConfig->autoInsertLRM && text.length() == 1 && editor->cursor().isRTL()) {
    if (editorViewConfig->autoInsertLRM && text.length() == 1 && S_test) {
        /*if (text.at(0) == '}') {
        ...
        if (std::find(charsToCheck.begin(), charsToCheck.end(), text.at(0)) != charsToCheck.end()) {
            bool autoOverride = editor->isAutoOverrideText(text.at(0));
            //bool previousIsLRM = editor->cursor().previousChar().unicode() == LRM;
            bool block = true;//previousIsLRM || autoOverride;
            if (block) editor->document()->beginMacro();
            //if (previousIsLRM) editor->cursor().deletePreviousChar(); //todo mirrors
            if (autoOverride) {
                editor->write(dirChar);
                editor->write(text.at(0)); //separated, so autooverride works
                editor->write(dirChar);
            } else editor->write("" + dirChar + text.at(0) + dirChar);
            if (block) editor->document()->endMacro();
            return true;
        }
	}
