...
void QEditor::mouseMoveEvent(QMouseEvent *e)
{
	...
	forever
	{
        ...
        //SSS
        std::vector<QString> charsToCheck = {"\\", "^", "-", "_", "+", "=", "!", "/", "#", "%", "*", " "};
        if (std::find(charsToCheck.begin(), charsToCheck.end(), newCursor.nextChar()) != charsToCheck.end()) {
            newCursor.movePosition(1, QDocumentCursor::Left);
        }
        if (std::find(charsToCheck.begin(), charsToCheck.end(), newCursor.previousChar()) != charsToCheck.end()) {
            newCursor.movePosition(1, QDocumentCursor::Right);
        }
...
void QEditor::mousePressEvent(QMouseEvent *e)
{
	...
	forever
	{
				...
				QApplication::startDragDistance() ))
		{
            ...
            //SSS
            std::vector<QString> charsToCheck = {"\\", "^", "-", "_", "+", "=", "!", "/", "#", "%", "*", " "};
            if (std::find(charsToCheck.begin(), charsToCheck.end(), cursor.nextChar()) != charsToCheck.end()) {
                cursor.movePosition(1, QDocumentCursor::Left);
            }
            if (std::find(charsToCheck.begin(), charsToCheck.end(), cursor.previousChar()) != charsToCheck.end()) {
                cursor.movePosition(1, QDocumentCursor::Right);
            }
...
void QEditor::cursorMoveOperation(QDocumentCursor &cursor, EditOperation eop){
	...
	case CursorLeft: case SelectCursorLeft:
        if(cursor.previousChar().unicode() == LRM)
            offset = 3;
	...
	case CursorRight: case SelectCursorRight:
        if(cursor.nextChar().unicode() == LRM)
            offset = 3;
    ...
    cursor.movePosition(offset, op, mode);
    std::vector<QString> charsToCheck = {"\\", "^", "-", "_", "+", "=", "!", "/", "#", "%", "*", " "};
    QChar previousChar = cursor.previousChar();
    QChar nextChar = cursor.nextChar();
    if (std::find(charsToCheck.begin(), charsToCheck.end(), nextChar) != charsToCheck.end()){
        cursor.movePosition(1, QDocumentCursor::Left, mode);
    }
    if (std::find(charsToCheck.begin(), charsToCheck.end(), previousChar) != charsToCheck.end()){
        cursor.movePosition(1, QDocumentCursor::Right, mode);
    }
}
...
void QEditor::processEditOperation(QDocumentCursor& c, const QKeyEvent* e, EditOperation op)
{
	...
	case DeleteLeft :
        if(!hasSelection) {
            QChar previousChar = c.previousChar();
            QChar nextChar = c.nextChar();
            if(previousChar.unicode() == LRM || previousChar.unicode() == 0x200F){
                //std::vector<QString> charsToCheck = {"\\", "^", "-", "_", "+", "=", "!", "/", "#", "%", "*", " "};
	...
	case DeleteRight :
        if(!hasSelection) {
            QChar previousChar = c.previousChar();
            QChar nextChar = c.nextChar();
            if(nextChar.unicode() == LRM || nextChar.unicode() == 0x200F){
                c.deleteChar();
                c.deleteChar();
                c.deleteChar();
            }
	...
	case DeleteLeftWord :
		...
		c.removeSelectedText();
        if(c.previousChar().unicode() == LRM || c.previousChar().unicode() == 0x200F){
            c.deletePreviousChar();
        }
		cutBuffer.clear();
		break;
	...
	case DeleteRightWord :
		...
		c.removeSelectedText();
        if(c.nextChar().unicode() == LRM || c.nextChar().unicode() == 0x200F){
            c.deleteChar();
        }
		cutBuffer.clear();
		break;
